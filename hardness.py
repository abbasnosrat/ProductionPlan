...
class OptimizationProblem:
    def __init__(self, products: pd.DataFrame, budget: float):
        ...
        self.hardness = products["HardnessCoefficient"].to_numpy()  # New hardness coefficient column
        self.bounds = []
        self.bounds = products.apply(get_bounds, axis=1).to_list()
        self.gain = lambda x: np.sum(x * self.benefit)
        self.hardness_priority = lambda x: np.sum(x * self.hardness)

    def objective(self, x):
        return -self.gain(x)  # You can combine gain and hardness in a weighted sum if needed
    ...
    def solve(self):
        ...
        total_weighted_benefit = solver.Sum(self.benefit[i] * self.hardness[i] * x[i] for i in range(num_products))
        
        solver.Maximize(total_weighted_benefit)
        ...
        if status == pywraplp.Solver.OPTIMAL:
            ...
            result = {
                ...
                'total_weighted_benefit': max_profit
            }
            return result
        else:
            return None
...
def main():
    ...
    Optimize your production plan by allocating your budget to maximize profits and prioritize hardness coefficient.
    """)
    ...
    if uploaded_file is not None:
        ...
        if solution:
            ...
            st.write(f"Total Weighted Benefit: {solution['total_weighted_benefit']:,.0f}")
